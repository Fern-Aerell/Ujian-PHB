...
const form = useForm({
    kepanjangan: props.kelasKategori?.kepanjangan ?? '',
    kependekan: props.kelasKategori?.kependekan ?? ''
});
...
function hapus(id: number) {
  Swal.fire(
    {
      title: "Pemberitahuan",
      text: `Yakin ingin menghapus ketegori kelas ${form.kependekan}?`,
      icon: "warning",
    ...
    (result) => {
      if (result.isConfirmed) {
        router.delete(route('config.kelas_kategori_data.delete', { id: id }), {
          onError: (error) => {
            failedAlert(error.message);
          },
          onSuccess: () => {
            successAlert(`Kategori kelas ${form.kependekan} berhasil dihapus, halaman akan direfresh untuk melihat perubahan.`, () => window.location.reload());
          },
        });
      }
    }
...
function tambah() {
  form.post(route('config.kelas_kategori_data.store'), {
    ...
    onSuccess: () => {
      form.reset();
      successAlert('Kategori kelas berhasil ditambahkan, halaman akan direfresh untuk melihat perubahan.', () => window.location.reload());
    },
  });
}
...
function kembali() {
    ...
    (result) => {
      if (result.isConfirmed) {
        ...
        isEdit.value = false;
        if(props.kelasKategori) {
            form.kepanjangan = props.kelasKategori.kepanjangan;
            form.kependekan = props.kelasKategori.kependekan;
        }
      }
    }
  );
...
function simpan(id: number) {
    form.post(route('config.kelas_kategori_data.update', {id: id}), {
        onError: (error) => {
            failedAlert(error.message);
        },
        onSuccess: () => {
            successAlert('Perubahan kategori kelas berhasil disimpan, halaman akan direfresh untuk melihat perubahan.', () => window.location.reload());
        },
    });
}
...
<template>
    <div v-if="!isHide" class="flex flex-col gap-2 border-gray-300 border p-3 w-full">
        ...
        <!-- EDIT MODE -->
        <template v-else>
            <form @submit.prevent="add? tambah() : props.kelasKategori ? simpan(props.kelasKategori.id) : undefined" class="flex flex-col gap-4">
                ...
                <div class="flex flex-row flex-wrap gap-2">
                    ...
                    <Button v-if="!add" @click="props.kelasKategori ? hapus(props.kelasKategori.id) : undefined" type="button" text="Hapus" bg-color="danger" text-color="white" class="!w-fit px-6" :class="{ 'opacity-25': form.processing }" :disabled="form.processing" />
                </div>
            </form>
        </template>
    </div>
