    ...
    <AuthLayout title="User List" class="flex flex-col gap-3">
        <div class="flex flex-col md:flex-row justify-between mb-4 gap-4">
            ...
            <div class="w-full md:w-auto">
                <Button @click="$inertia.get(route('user.add'))" text="Tambah User" text-color="white" bg-color="primary" class="w-full md:w-fit px-4 py-2"/>
            </div>
            <!-- Search and Filter -->
            <div class="flex flex-col md:flex-row w-full md:w-auto gap-2">
                <input type="text" placeholder="Cari user..." class="w-full md:w-48 lg:w-64 px-3 py-2 border" @input="$inertia.get($page.url, { search: ($event.target as any).value }, { preserveState: true, replace: true })">
                <select class="w-full md:w-32 px-3 py-2 border" @change="$inertia.get($page.url, { type: ($event.target as any).value }, { preserveState: true, replace: true })">
                    <option value="semua">Semua</option>
                    <option value="murid">Murid</option>
                    <option value="guru">Guru</option>
                    <option value="admin">Admin</option>
                </select>
                <select class="w-full md:w-40 px-3 py-2 border" @change="$inertia.get($page.url, { max: ($event.target as any).value }, { preserveState: true, replace: true })">
                    ...
                    <tr v-for="(user, index) in props.user_list.data" :key="index" class="border-b hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2">
                            <div class="flex flex-col sm:flex-row gap-2">
                                ...
                                <Button @click="deleteUser(user.username, user.id)" text="Hapus" text-color="white" bg-color="danger" class="w-full sm:w-fit px-3 py-1 text-sm"/>
                            </div>
                        </td>
                    </tr>
                </tbody>
